...
export async function listMembers(page = 1, limit = 10, filtersOrQ: string | Record<string, unknown> = {}) {
  const skip = (page - 1) * limit;
  let query: Record<string, unknown> = {};
  if (typeof filtersOrQ === 'string' && filtersOrQ.trim()) {
    const q = filtersOrQ.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    query = { $or: [{ nombre: { $regex: q, $options: 'i' } }, { email: { $regex: q, $options: 'i' } }] } as Record<string, unknown>;
